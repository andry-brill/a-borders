...
void main() {
  test('derives tab insets from bottom corner extents', () {
  ...
  test('normalizes oversized bottom corner extents to fit width', () {
    ...
    expect(points[1].point.dx, 60);
    expect(points[4].point.dx, 60);
  });
  ...
  test('lerp keeps collapsed tab point when only one side skips it', () {
    ...
    expect(points, hasLength(6));
    expect(points[1].skip, isFalse);
    expect(points[1].point, const Offset(5, 40));
  });
