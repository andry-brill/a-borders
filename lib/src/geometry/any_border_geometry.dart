...
class AnyBorderGeometry {
  ...
  static AnyResolvedSide _resolveSide(IAnySide side) {
    switch (side.align) {
      ...
      case AnyAlign.center:
        return AnyResolvedSide(side: side, outside: side.width / 2.0, inside: side.width / 2.0);
  ...
  static AnyContour _makeContour(Rect rect, IAnyBorder border) {
    ...
    return AnyContour(
      rect: rect,
      topLeft: AnyCornerProfile.fromCorner(border.topLeft, AnyCornerPosition.topLeft, size),
      topRight: AnyCornerProfile.fromCorner(border.topRight, AnyCornerPosition.topRight, size),
      bottomRight: AnyCornerProfile.fromCorner(border.bottomRight, AnyCornerPosition.bottomRight, size),
      bottomLeft: AnyCornerProfile.fromCorner(border.bottomLeft, AnyCornerPosition.bottomLeft, size),
    );
  }
  ...
  List<AnyRegion> buildVisibleRegions(AnyDecoration decoration) {
    ...
    _addSideRegion(regions, decoration.border.top, decoration, _SideId.top);
    _addSideRegion(regions, decoration.border.right, decoration, _SideId.right);
    _addSideRegion(regions, decoration.border.bottom, decoration, _SideId.bottom);
    _addSideRegion(regions, decoration.border.left, decoration, _SideId.left);
    return regions;
  }

  Path buildMergedBackgroundPath(AnyDecoration decoration) {
    final useTopOuter = decoration.border.top.isSameAs(decoration);
    final useRightOuter = decoration.border.right.isSameAs(decoration);
    final useBottomOuter = decoration.border.bottom.isSameAs(decoration);
    final useLeftOuter = decoration.border.left.isSameAs(decoration);
    ...
    _appendSide(path, _SideId.top, useTopOuter ? outerContour : baseContour, fromMiddle: true);
    _appendJoin(path, AnyCornerPosition.topRight, decoration, useTopOuter, useRightOuter);
    _appendSide(path, _SideId.right, useRightOuter ? outerContour : baseContour);
    _appendJoin(path, AnyCornerPosition.bottomRight, decoration, useRightOuter, useBottomOuter);
    _appendSide(path, _SideId.bottom, useBottomOuter ? outerContour : baseContour);
    _appendJoin(path, AnyCornerPosition.bottomLeft, decoration, useBottomOuter, useLeftOuter);
    _appendSide(path, _SideId.left, useLeftOuter ? outerContour : baseContour);
    _appendJoin(path, AnyCornerPosition.topLeft, decoration, useLeftOuter, useTopOuter);
    path.close();
    return path;
  }
  ...
  void _addSideRegion(
    ...
    if (side.width <= 0.0 || side.isEmpty) return;
    if (side.isSameAs(decoration) && decoration.hasBaseFill) return;

    final path = _buildSideStripPath(id);
    out.add(AnyRegion(path: path, fill: side, debugLabel: 'side:$id'));
  }
  ...
  Path _buildSideStripPath(_SideId id) {
    ...
    switch (id) {
      case _SideId.top:
        ...
        _appendCornerBridge(path, AnyCornerPosition.topRight, outerContour, innerContour);
        path.lineTo(innerContour.topStart().dx, innerContour.topStart().dy);
        _appendCornerBridge(path, AnyCornerPosition.topLeft, innerContour, outerContour, reverse: true);
        path.close();
        break;
      case _SideId.right:
        ...
        _appendCornerBridge(path, AnyCornerPosition.bottomRight, outerContour, innerContour);
        path.lineTo(innerContour.rightStart().dx, innerContour.rightStart().dy);
        _appendCornerBridge(path, AnyCornerPosition.topRight, innerContour, outerContour, reverse: true);
        path.close();
        break;
      case _SideId.bottom:
        ...
        _appendCornerBridge(path, AnyCornerPosition.bottomLeft, outerContour, innerContour);
        path.lineTo(innerContour.bottomStart().dx, innerContour.bottomStart().dy);
        _appendCornerBridge(path, AnyCornerPosition.bottomRight, innerContour, outerContour, reverse: true);
        path.close();
        break;
      case _SideId.left:
        ...
        _appendCornerBridge(path, AnyCornerPosition.topLeft, outerContour, innerContour);
        path.lineTo(innerContour.leftStart().dx, innerContour.leftStart().dy);
        _appendCornerBridge(path, AnyCornerPosition.bottomLeft, innerContour, outerContour, reverse: true);
        path.close();
        break;
    }
    return path;
  ...
  void _appendSide(Path path, _SideId id, AnyContour contour, {bool fromMiddle = false}) {
    switch (id) {
      case _SideId.top:
  ...
  void _appendJoin(
    ...
    if (currentUsesOuter == nextUsesOuter) {
      _appendCornerContour(path, position, currentUsesOuter ? outerContour : baseContour, clockwise: true);
      return;
    }
  ...
  void _appendCornerContour(Path path, AnyCornerPosition pos, AnyContour contour, {required bool clockwise}) {
    switch (pos) {
  ...
  Offset _cornerEndPoint(AnyCornerPosition pos, AnyContour contour, {required bool clockwise}) {
    switch (pos) {
      case AnyCornerPosition.topLeft:
  ...
  Offset _cornerStartPoint(AnyCornerPosition pos, AnyContour contour, {required bool clockwise}) {
    switch (pos) {
      case AnyCornerPosition.topLeft:
